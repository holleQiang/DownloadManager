...
public class VisitRecordFragment extends BaseFragment {
    ...
    public static VisitRecordFragment newInstance(String sessionId) {
    ...
    private void refreshHistoryList() {
        Observable.create(new ObservableOnSubscribe<List<VisitRecordBean>>() {
        ...
        }).compose(RXJavaUtils.applyIOMainSchedules()).compose(RXJavaUtils.bindLifecycle(this)).map(new Function<List<VisitRecordBean>, List<Cell>>() {
            @Override
            public List<Cell> apply(List<VisitRecordBean> visitRecordBeans) throws Exception {
                ...
                for (VisitRecordBean visitRecordBean : visitRecordBeans) {
                    ...
                    cells.add(new VisitRecordCell(visitRecordBean, onVisitRecordClickListener, new VisitRecordCell.OnVisitRecordLongClickListener() {
                        @Override
                        public void onVisitRecordLongClick(VisitRecordBean data, int position) {
                            ...
                            new AlertDialog.Builder(activity).setTitle(R.string.tips).setMessage(R.string.delete_record).setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
                                @Override
    ...
    public void setOnVisitRecordClickListener(OnVisitRecordClickListener onVisitRecordClickListener) {
